...
def kodto(kod,kodlista,default):
...
def vanbenne(strIn,samples,bWholeword=False):
...
def cutright(strIn,strCut):
    # Nem ellenőrzi, hogy valóban a strCut-tal végződik-e, egyszerűen levágja a végéről a strCut hosszának megfelelő karaktereket
    return strIn[:-len(strCut)]

def txtshorten(text,maxlen,rightlen=0):
...
def nextpath(path_pattern,downloads=True):
    """
    A következő (még nem foglalt) sorszámozott fájlnevet adja vissza.
    Gyors kereső-algoritmus, nagy tömegű fájl esetén is.
    Requirement: import os

    fname_pattern:   példa  'filename (%s).txt'     A sorszám helyén %s jel.  Teljes path is megadható
    downloads:  True esetén eléírja a downloads mappa elérési útját 
    """
    if downloads:
        path_pattern="c:\\users\\zsolt\\downloads\\" + path_pattern
